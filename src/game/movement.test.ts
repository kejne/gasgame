...
import { CITY_MAP, isWalkable } from './map';
...
describe('grid movement', () => {
  ...
  it('moves one walkable tile and preserves the cardinal facing', () => {
  ...
  it('keeps repeated actions on integer coordinates and cardinal facings', () => {
    ...
    expect(Number.isInteger(current.tile.x)).toBe(true);
    expect(Number.isInteger(current.tile.y)).toBe(true);
